...
2.2. Заказчик оплачивает 100% стоимости Изделия на счёт платформы Asia Mebel в рамках сервиса «Безопасная сделка» единовременно, после подписания настоящего Договора обеими Сторонами.
2.3. Оплаченная сумма удерживается платформой и становится доступна Исполнителю на балансе платформы для запроса перевода не сразу, а поэтапно: ${deal.interimPercent > 0 ? 'аванс' : 'предоплата'} в размере ${deal.prepaymentPercent}% — сразу после оплаты Заказчиком${deal.interimPercent > 0 ? `; промежуточный транш в размере ${deal.interimPercent}% — по запросу Исполнителя в период изготовления Изделия` : ''}; окончательный транш в размере ${deal.finalPercent}% — после подписания Сторонами акта приёма-передачи.
